...
app.defineConsensusClass(((app) => {
    class PosConsensus extends app.CONSENSUS.ProofOfStakeConsensus {
        constructor() {
            super("Example PoS test consensus", "examplepos")//must append examplepos section in config before start
        }
...
for (; ;) {
    ...
    process.stdout.write("\x1b[32m" + parseFloat((Date.now() - time) / 1000).toFixed(2) + "\x1b[0m " + data.id + "\n");
}